<?php
// File: tag.blade.php
// Path: /resources/views/news/tag.blade.php
?>

@extends('layouts.app')

@section('title', '#' . $tag . ' Articles - Pet News')
@section('meta_description', 'Browse articles tagged with ' . $tag . ' for pet care tips, health advice, and wellness information.')
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div class="text-center">
            ...
            <p class="text-xl md:text-2xl text-purple-100 mb-8">
                {{ $news->total() }} {{ Str::plural('article', $news->total()) }} tagged with 
            ...
            <div class="max-w-2xl mx-auto">
                <form action="{{ route('news.search') }}" method="GET" class="flex">
                    <input type="text" name="q" placeholder="Search in #{{ $tag }} articles..." 
        ...
        <div class="grid grid-cols-1 lg:grid-cols-4 gap-8">
            ...
            <div class="lg:col-span-3">
                ...
                <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-8">
                    <form method="GET" action="{{ route('news.tag', $tag) }}" class="flex flex-wrap items-center gap-4">
                        <div class="flex-1 min-w-0">
                            <div class="relative">
                                <svg class="w-5 h-5 absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z"/>
                                </svg>
                                <input type="text" name="search" placeholder="Search in #{{ $tag }} articles..." 
                        ...
                        @if(request()->hasAny(['search', 'sort']))
                            <a href="{{ route('news.tag', $tag) }}" class="px-6 py-2 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-lg transition duration-200">
                                Reset
                            </a>
                        @endif
                    </form>
                ...
                <div class="flex items-center justify-between mb-6">
                    <div class="text-gray-600">
                        <span class="font-medium">{{ $news->total() }}</span> {{ Str::plural('article', $news->total()) }} tagged with 
                                ...
                                <div class="p-6">
                                    ...
                                    @if($article->tags && count($article->tags) > 0)
                                        <div class="flex flex-wrap gap-2 mb-4">
                                            @foreach(array_slice($article->tags, 0, 4) as $articleTag)
                                                <a href="{{ route('news.tag', $articleTag) }}" 
                                                   class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium transition duration-200 {{ $articleTag === $tag ? 'bg-purple-100 text-purple-800' : 'bg-gray-100 text-gray-700 hover:bg-gray-200' }}">
                                                    #{{ $articleTag }}
                                                </a>
                                            @endforeach
                                        </div>
                ...
                @else
                    ...
                    <div class="text-center py-12">
                        ...
                        <p class="text-gray-600 mb-4">
                            @if(request('search'))
                                No articles found matching your search in #{{ $tag }}.
                            @else
                                No articles available with this tag yet.
                            @endif
                        </p>
                        <div class="flex items-center justify-center space-x-3">
                            @if(request('search'))
                                <a href="{{ route('news.tag', $tag) }}" class="inline-flex items-center px-4 py-2 bg-purple-600 hover:bg-purple-700 text-white rounded-lg transition duration-200">
            ...
            <div class="lg:col-span-1">
                ...
                @if($relatedTags->count() > 0)
                    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-8">
                        <h3 class="text-lg font-bold text-gray-900 mb-4">Related Tags</h3>
                        <div class="flex flex-wrap gap-2">
                            @foreach($relatedTags as $relatedTag)
                                <a href="{{ route('news.tag', $relatedTag) }}" 
                ...
                <!-- Tag Actions -->
                <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-8">
                    <h3 class="text-lg font-bold text-gray-900 mb-4">Tag Actions</h3>
                    <div class="space-y-3">
                        <a href="{{ route('news.search', ['q' => $tag]) }}" 
                           class="flex items-center px-4 py-3 bg-gray-50 hover:bg-gray-100 rounded-lg transition duration-200">
                            <svg class="w-5 h-5 mr-3 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/>
                            </svg>
                            <span class="text-gray-700">Search "{{ $tag }}"</span>
                        </a>
                ...
                <div class="bg-purple-50 border border-purple-200 rounded-lg p-6 mb-8">
                    <h3 class="text-lg font-bold text-purple-900 mb-3">About This Tag</h3>
                    <div class="space-y-2 text-sm text-purple-800">
                        <div class="flex items-center justify-between">
                            <span>Total Articles:</span>
                            <span class="font-semibold">{{ $news->total() }}</span>
                        </div>
                        <div class="flex items-center justify-between">
                            <span>Tag:</span>
                        ...
                        @if($relatedTags->count() > 0)
                            <div class="flex items-center justify-between">
                                ...
                                <span class="font-semibold">{{ $relatedTags->count() }}</span>
                            </div>
                        @endif
                    </div>
                </div>
                ...
                <div class="bg-purple-600 text-white rounded-lg p-6">
                    ...
                    <p class="text-purple-100 text-sm mb-4">Get notified when new #{{ $tag }} articles are published</p>
                    
                    <form id="tag-newsletter-form">
...
document.getElementById('tag-newsletter-form').addEventListener('submit', function(e) {
    e.preventDefault();
